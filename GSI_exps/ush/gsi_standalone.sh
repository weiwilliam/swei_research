...
if [[ $SINGLERADOB == ".true." ]]; then
   DIAGRADJAC='.true.'
   SINGLEOB_TEST="oneob_type='${ONEOBTYPE}',maginnov=1.,oblat=${ONEOBLAT},oblon=${ONEOBLON},obchan=${ONEOBCHAN}"
else
   DIAGRADJAC='.false.'
   SINGLEOB_TEST=""
fi
...
export JCAP_A=$((res*2-2))
export NLON_A=$((res*4))
export NLAT_A=$((res*2+2))
...
cat << EOF > gsiparm.anl
 ...
 &SETUP
   miter=2,niter(1)=100,niter(2)=50,
