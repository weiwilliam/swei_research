...
if [ ${if_iraerdet} = Yes ] ; then
  OBSQC="ir_aer_det=.ture.,${OBSQC}"
else                                
  OBSQC="ir_aer_det=.false.,${OBSQC}"
fi
if [ ${if_useaerir} = Yes ] ; then
  OBSQC="luse_aerobs=.true.,${OBSQC}"
else
  OBSQC="ir_aer_det=.false.,${OBSQC}"
fi

if [ ${if_hybvar} = Yes ]; then
   l_hyb_ens=.true.
...
p3date=`$ndate  3 $CDATE`
p2date=`$ndate  2 $CDATE`
p1date=`$ndate  1 $CDATE`
m1date=`$ndate -1 $CDATE`
m2date=`$ndate -2 $CDATE`
m3date=`$ndate -3 $CDATE`
p3yy=${p3date:0:4}; p3mm=${p3date:4:2}
p2yy=${p2date:0:4}; p2mm=${p2date:4:2}
p1yy=${p1date:0:4}; p1mm=${p1date:4:2}
m1yy=${m1date:0:4}; m1mm=${m1date:4:2}
m2yy=${m2date:0:4}; m2mm=${m2date:4:2}
m3yy=${m3date:0:4}; m3mm=${m3date:4:2}
...
aer03=${aerpath}/$m3yy/$m3mm/MERRA2_AER3D_FV3L64.${m3date}.nc
aer04=${aerpath}/$m3yy/$m3mm/MERRA2_AER3D_FV3L64.${m2date}.nc
aer05=${aerpath}/$m3yy/$m3mm/MERRA2_AER3D_FV3L64.${m1date}.nc
aer06=${aerpath}/$ayy/$amm/MERRA2_AER3D_FV3L64.${CDATE}.nc
aer07=${aerpath}/$p3yy/$p3mm/MERRA2_AER3D_FV3L64.${p1date}.nc
aer08=${aerpath}/$p3yy/$p3mm/MERRA2_AER3D_FV3L64.${p2date}.nc
aer09=${aerpath}/$p3yy/$p3mm/MERRA2_AER3D_FV3L64.${p3date}.nc

#datobs=${homedir}/INPUTS/OBSDATA/${cdump}.$PDY/$cyc
datobs=${obsarch}/${cdump}.$PDY/$cyc
datges=${gesarch}/${cdump}.$gPDY/$gcyc/atmos
datges_enkf=${gesarch}/enkf${cdump}.$gPDY/$gcyc/atmos
...
cat << EOF > gsiparm.anl
 ...
 &SETUP
  ...
  diag_precon=.true.,step_start=1.e-3,emiss_bc=.true.,nhr_obsbin=1,
...
$ncpl $datges/${prefix_inp}.sfcf004.${sfcsuffix}       ./sfcf04
$ncpl $datges/${prefix_inp}.sfcf005.${sfcsuffix}       ./sfcf05
$ncpl $datges/${prefix_inp}.sfcf006.${sfcsuffix}       ./sfcf06
$ncpl $datges/${prefix_inp}.sfcf007.${sfcsuffix}       ./sfcf07
$ncpl $datges/${prefix_inp}.sfcf008.${sfcsuffix}       ./sfcf08
$ncpl $datges/${prefix_inp}.sfcf009.${sfcsuffix}       ./sfcf09

$ncpl $datges/${prefix_inp}.atmf003.${atmsuffix}       ./sigf03
$ncpl $datges/${prefix_inp}.atmf004.${atmsuffix}       ./sigf04
$ncpl $datges/${prefix_inp}.atmf005.${atmsuffix}       ./sigf05
$ncpl $datges/${prefix_inp}.atmf006.${atmsuffix}       ./sigf06
$ncpl $datges/${prefix_inp}.atmf007.${atmsuffix}       ./sigf07
$ncpl $datges/${prefix_inp}.atmf008.${atmsuffix}       ./sigf08
...
if [ $if_hybvar = Yes ]; then
   ...
   for imem in $(seq 1 $NMEM_ENKF); do
      ...
      for fhr in $fhrs; do
         $ncpl ${datges_enkf}/$memchar/${prefix_inp}.atmf0${fhr}.${enkfsuffix} ./ensemble_data/sigf${fhr}_ens_$memchar
      done
   done

   # use_readin_anl_sfcmask=.true. in gsiparm.anl
   $ncpl ${datges_enkf}/${prefix_inp}.sfcf006.ensmean.${enkfsuffix} ./sfcf06_anlgrid
fi
