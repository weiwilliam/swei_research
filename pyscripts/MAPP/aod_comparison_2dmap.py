...
def hofx_dict(ncd):
    ...
    ichan_wave0 = np.where( abs(wavelengths - wave0) < small )[0][0]
    outdict = {
               'lats':   ncd.groups['MetaData'].variables['latitude' ][:],
               'lons':   ncd.groups['MetaData'].variables['longitude'][:],
               'obs':    ncd.groups['ObsValue'].variables['aerosol_optical_depth'][:,ichan_wave0],
               'hofx':   ncd.groups['hofx'].variables['aerosol_optical_depth'][:,ichan_wave0],
               'preqc':  ncd.groups['PreQC'].variables['aerosol_optical_depth'][:,ichan_wave0],
               'effqc':  ncd.groups['EffectiveQC'].variables['aerosol_optical_depth'][:,ichan_wave0],
               }
    return outdict
...
for cdate in work_dates:
    ...
    file1 = ('%s/%s_%s/aeronet_aod_hofx.%s.%s' % (archdir,explist[0],obtype,cdatestr,suffix))
    file2 = ('%s/%s_%s/aeronet_aod_hofx.%s.%s' % (archdir,explist[1],obtype,cdatestr,suffix))
    ...
    df1 = process_aod(dict1)
    df2 = process_aod(dict2)
