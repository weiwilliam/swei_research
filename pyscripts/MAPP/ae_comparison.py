...
def plt_hist2d(hist2d, axis, stats_dict, xmax, lvs, norm, savedir, expname, timetag):
    xlabstr='AERONET AE 440-870 nm'
    ylabstr='GEFS AE 440-870 nm'
...
def calculate_ae(in_dict,wave0,wave1,wave2):
    ichan_wave1 = np.where( abs(in_dict['wavelengths'].data - wave1) < small )[0][0]
    ichan_wave0 = np.where( abs(in_dict['wavelengths'].data - wave0) < small )[0][0]
    ichan_wave2 = np.where( abs(in_dict['wavelengths'].data - wave2) < small )[0][0]

    effqc_mask_wave1 = in_dict['effqc'].data[:,ichan_wave1] == 0
    effqc_mask_wave0 = in_dict['effqc'].data[:,ichan_wave0] == 0
    effqc_mask_wave2 = in_dict['effqc'].data[:,ichan_wave2] == 0
    obs_mask_wave1 = in_dict['obs'].data[:,ichan_wave1] >= threshold_wave1

    effqc_mask = (effqc_mask_wave1 & effqc_mask_wave0 & effqc_mask_wave2 & obs_mask_wave1)

    ae_wave1_wave2_obs = -np.log(in_dict['obs'].data[effqc_mask,ichan_wave1]/
                                 in_dict['obs'].data[effqc_mask,ichan_wave2])/np.log(wave1/wave2)

    ae_wave1_wave2_hofx = -np.log(in_dict['hofx'].data[effqc_mask,ichan_wave1]/
                                  in_dict['hofx'].data[effqc_mask,ichan_wave2])/np.log(wave1/wave2)

    aod_wave0_obs = in_dict['obs'][effqc_mask,ichan_wave0]
    aod_wave1_obs = in_dict['obs'][effqc_mask,ichan_wave1]
    aod_wave2_obs = in_dict['obs'][effqc_mask,ichan_wave2]

    aod_wave0_hofx = in_dict['hofx'][effqc_mask,ichan_wave0]
    aod_wave1_hofx = in_dict['hofx'][effqc_mask,ichan_wave1]
    aod_wave2_hofx = in_dict['hofx'][effqc_mask,ichan_wave2]

    return ae_wave1_wave2_obs, ae_wave1_wave2_hofx, effqc_mask
...
for cdate in work_dates:
    ...
    ae_obs1, ae_hofx1, mask1 = calculate_ae(dict1,wave0,wave1,wave2)
    ae_obs2, ae_hofx2, mask2 = calculate_ae(dict2,wave0,wave1,wave2)

    if cdate == work_dates[0]:
       ae_obs1_all = ae_obs1
       ae_obs2_all = ae_obs2
       ae_hofx1_all = ae_hofx1
       ae_hofx2_all = ae_hofx2
    else:
       ae_obs1_all = np.append(ae_obs1_all,ae_obs1)
       ae_obs2_all = np.append(ae_obs2_all,ae_obs2)
       ae_hofx1_all = np.append(ae_hofx1_all,ae_hofx1)
       ae_hofx2_all = np.append(ae_hofx2_all,ae_hofx2)
