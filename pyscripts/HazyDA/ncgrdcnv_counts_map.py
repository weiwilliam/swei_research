...
cnts0=tmpds0['omb_bc_count'].sum(dim=sum_dims)
cnts1=tmpds1['omb_bc_count'].sum(dim=sum_dims)
#
#
#
if (plt_fig):
   cnts_cnlvs=find_cnlvs(xa.concat((cnts0,cnts1),dim='exps'),ntcks=21,eqside=0)
   clridx=[]
   for idx in np.linspace(2,254,cnts_cnlvs.size):
       clridx.append(int(idx))
   cnts_clmap=setup_cmap('WhiteYellowOrangeRed',clridx)
   cnts_norm = mpcrs.BoundaryNorm(cnts_cnlvs,len(clridx)+1,extend='both')
   #
   cnts_diff_cnlvs=find_cnlvs(cnts1-cnts0,ntcks=21,eqside=1)
   clridx=[]
   for idx in np.linspace(2,254,cnts_diff_cnlvs.size):
       clridx.append(int(idx))
   cnts_diff_clmap=setup_cmap('BlueYellowRed',clridx)
   cnts_diff_norm = mpcrs.BoundaryNorm(cnts_diff_cnlvs,len(clridx)+1,extend='both')
   #
   #
   #
   if grdtype == 'time':
   ...
   cblabel='%s counts %s' %(expnlist[0],cnvtype_str)
   cb_dict['label']=cblabel
   plt2d_contourf(fig,ax,cnts0,cnts_cnlvs,cnts_clmap,cnts_norm,units,cb_dict)
   outname='%s/%s_%s_counts_%s.%s_%s.%s' %(imgsavpath,expnlist[0],cnvtype_str,tlstr,sdate_fstr,edate_fstr,ffmt)
   ...
   cblabel='%s counts %s' %(expnlist[0],cnvtype_str)
   cb_dict['label']=cblabel
   plt2d_contourf(fig,ax,cnts1,cnts_cnlvs,cnts_clmap,cnts_norm,units,cb_dict)
   outname='%s/%s_%s_counts_%s.%s_%s.%s' %(imgsavpath,expnlist[1],cnvtype_str,tlstr,sdate_fstr,edate_fstr,ffmt)
   if (fsave): print(outname,flush=1); fig.savefig(outname,dpi=quality) ; plt.close()
   #
   fig,ax,gl=setupax_2dmap(cornerll,area,proj,lbsize=txsize)
   set_size(axe_w,axe_h,b=0.15,l=0.05,r=0.95)
   cblabel='%s%s%s counts %s' %(expnlist[1],minussign,expnlist[0],cnvtype_str)
   cb_dict['label']=cblabel
   plt2d_contourf(fig,ax,cnts1-cnts0,cnts_diff_cnlvs,cnts_diff_clmap,cnts_diff_norm,units,cb_dict)
   outname='%s/%s-%s_%s_counts_%s.%s_%s.%s' %(imgsavpath,expnlist[1],expnlist[0],cnvtype_str,tlstr,sdate_fstr,edate_fstr,ffmt)
   if (fsave): print(outname,flush=1); fig.savefig(outname,dpi=quality) ; plt.close()
