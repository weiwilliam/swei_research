...
import os
import sys
sys.path.append('/Users/weiwilliam/AlbanyWork/Utility/Python3/fortran')
...
outpath='/Users/weiwilliam/AlbanyWork/R2O_Project/images/tmp/Conventional/Scatter'
...
area='ArbS'# Glb, NPO, NML, TRO, SML, SPO, EAsia, NAfr
...
for var in varlist:
    ...
    if (var=='ps' or var=='sst' or var=='tcp'):
        ...
        for date in dlist:
            ...
            obs1=ds1.obsdata[16,:] 
            obs2=ds2.obsdata[16,:] 
            mdl1=obs1-ds1.obsdata[17,:] # O-(Innov(O-F,O-A))=(F,A)
            mdl2=obs2-ds2.obsdata[17,:]
            
            try:
                scatobs1
            except NameError:
                scatobs1=obs1 ; scatmdl1=mdl1 ; lon1=rlon1 ; lat1=rlat1
                scatobs2=obs2 ; scatmdl2=mdl2 ; lon2=rlon2 ; lat2=rlat2
            else:
                scatobs1=xa.concat((scatobs1,obs1),dim='obsloc')
                scatmdl1=xa.concat((scatmdl1,mdl1),dim='obsloc')
                lon1=xa.concat((lon1,rlon1),dim='obsloc')
                lat1=xa.concat((lat1,rlat1),dim='obsloc')
                scatobs2=xa.concat((scatobs2,obs2),dim='obsloc')
                scatmdl2=xa.concat((scatmdl2,mdl2),dim='obsloc')
                lon2=xa.concat((lon2,rlon2),dim='obsloc')
                lat2=xa.concat((lat2,rlat2),dim='obsloc')        

        mask1=~np.isnan(scatobs1.obsloc)
        mask2=~np.isnan(scatobs2.obsloc)

        if (area!='Glb'):
            mask1=(mask1)&((lon1<maxlon)&(lon1>minlon)&(lat1>minlat)&(lat1<maxlat))
            mask2=(mask2)&((lon2<maxlon)&(lon2>minlon)&(lat2>minlat)&(lat2<maxlat))
        
        if (useqc):
            mask1=(mask1)&(iuse1==1)
            mask2=(mask2)&(iuse2==1)
      
        pltidx1=np.where(mask1==1)[0] 
        pltidx2=np.where(mask2==1)[0] 

        pltobs1=scatobs1[pltidx1]
        pltobs2=scatobs2[pltidx2]
        pltmdl1=scatmdl1[pltidx1]
        pltmdl2=scatmdl2[pltidx2]
        ...
        fname='%s/%s_%s_%s_%s_%s_%s_%s.%s'%(outpath,area,expnlist[0],expnlist[1],var.upper(),
                                      tlstr,qcflg,pltidx1.size,ffmt)
