...
edate=2020071018
hint=6
explist=['hazyda_ctrl','hazyda_aerov6']
expnlist=['CTL','AERv2']
sensor='iasi_metop-a'
chkwvn_list=[962.5]
loop='ges' #ges,anl
...
savedir=outpath+'/DiagFiles/rad/pdf_2exps/'+area
...
for date in dlist:
    ...
    infile1=archdir1+'/'+str(date)+'/'+raddfile0
...
for chkwvn in chkwvn_list:
    ...
    if (plthist):            
        x_label=tlstr
        y_label='Data Count'
        pltda_x1=omb_bc0[passed_msk0]
        pltda_x2=omb_bc1[passed_msk1]
        ...
        pltda_x3=omb_nbc0[passed_msk0]
        pltda_x4=omb_nbc1[passed_msk1]
        ...
        ax.plot(bin_center,hdata1,'b',ls='-',lw=1.5,label=expnlist[0]+' w/ BC')
        ax.plot(bin_center,hdata2,'r',ls='-',lw=1.5,label=expnlist[1]+' w/ BC')
        ax.plot(bin_center,hdata3,'b',ls='--',lw=1.5,label=expnlist[0]+' w/o BC')
        ax.plot(bin_center,hdata4,'r',ls='--',lw=1.5,label=expnlist[1]+' w/o BC')
        ...
        if (fsave):
            fname=('%s/PDF_%s_%s_%s_%.2f.%s_%s.%s'
                    %(savedir,exps_fname_str,tlstr,sensor,chkwvn,sdate,edate,ffmt))
            print(fname,flush=1)
