...
for date in dlist:
    ...
    for qcval in qclist:
        ...
        tmpdf0_qcfilter=((tmpdf0['qcflag']==qcval))
        ...
        tmpgrp0 = tmpoutdf0.groupby(['wavenumber','lat','lon']).agg({'qcflag':['count']})
    
        tmpgrp0 = tmpgrp0.rename(columns={'qcflag':qcname})
    
        if (qcidx==0):
           tmpoutgrp0=tmpgrp0
        else:
           tmpoutgrp0=pd.concat((tmpoutgrp0,tmpgrp0),axis=1)
        qcidx+=1

    tmpgrdds0=tmpoutgrp0.to_xarray()
...
fname0='%s/%s_%s_%s_qcflags_%.1fx%.1f.time.%s_%s.nc' %(outpath,leglist[0],sensor,loop,degres,degres,sdate,edate)
...
print(total_obscounts0,flush=1)
...
#
fname0='%s/%s_%s_%s_qcflags_%.1fx%.1f.mean.%s_%s.nc' %(outpath,leglist[0],sensor,loop,degres,degres,sdate,edate)
print(fname0,flush=1)
