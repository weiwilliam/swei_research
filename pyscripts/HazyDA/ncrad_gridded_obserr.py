...
for date in dlist:
    ...
    varinv0=ds0.Inverse_Observation_Error
    obserr0=np.reshape(xa.where(varinv0!=0.,1/varinv0,np.nan).values,(npts0,nchs0))

    tmpds0=xa.Dataset({'rlon':(['obsloc'],rlon0),
                       'rlat':(['obsloc'],rlat0),
                       'qcflag':(['obsloc','wavenumber'],qcflags0),
                       'obserr':(['obsloc','wavenumber'],obserr0)
                       },
    ...
    tmpgrp0 = tmpoutdf0.groupby(['wavenumber','lat','lon']).agg({'obserr':statslist,})

    tmpgrdds0=tmpgrp0.to_xarray()

    for stats in statslist:
       newname='%s_%s'%('obserr',stats)
       tmpgrdds0=tmpgrdds0.rename({('obserr',stats):(newname)})
...
fname0='%s/%s_%s_%s_%s_obserr_%.1fx%.1f.time.%s_%s.nc' %(outpath,leglist[0],sensor,loop,qcflg,degres,degres,sdate,edate)
...
grp0 = outdf0.groupby(['wavenumber','lat','lon']).agg({'obserr':statslist,})

grdds0=grp0.to_xarray()

for stats in statslist:
   newname='%s_%s'%('obserr',stats)
   grdds0=grdds0.rename({('obserr',stats):(newname)})

fname1='%s/%s_%s_%s_%s_obserr_%.1fx%.1f.mean.%s_%s.nc' %(outpath,leglist[0],sensor,loop,qcflg,degres,degres,sdate,edate)
