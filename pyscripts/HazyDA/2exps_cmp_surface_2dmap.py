...
for date in dlist:
    ...
    exp0grb=expsarch+'/'+explist[0]+'/'+date+'/pgbf00.gdas.'+date+'.grib2'
    exp1grb=expsarch+'/'+explist[1]+'/'+date+'/pgbf00.gdas.'+date+'.grib2'

    exp0=xa.open_dataset(exp0grb,filter_by_keys={"typeOfLevel": "surface"})
    exp1=xa.open_dataset(exp1grb,filter_by_keys={"typeOfLevel": "surface"})
...
diff_da=exp_diff.mean(dim='time')
...
outname='%s/%s_%s_%s_sfc_Diff.%s_%s.%s' %(imgsavpath,expnlist[1],expnlist[0],pltvar,sdate,edate,ffmt)
if (fsave):
   print(outname)
