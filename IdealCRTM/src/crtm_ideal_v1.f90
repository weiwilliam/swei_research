...
IMPLICIT NONE
  ...
  INTEGER :: nblvs, kk, nlvs_process, st_proc_lv
  REAL(fp) :: abovemode, belowmode, alvsmean, blvsmean, modeinc
  REAL(fp) :: degint_ab, degint_bl
  REAL(fp), ALLOCATABLE :: degs(:),cosvalue(:),weighting(:)
  ...
  namelist/autogen/varname,genmethod,totalconc,thicknesslvs,atlv,modelv,bmoderatio,sfc_mode_ratio
  ! Aerosol Profile
  CHARACTER(len=256) :: aername
  INTEGER :: aerlvs
  ...
  ! Interpolation
     ...
     aername=varname
        IF (genmethod .eq. 1) THEN
           ...
           atm(1)%Aerosol(1)%Concentration(atlv:atlv+thicknesslvs-1)=totalconc/thicknesslvs
        ELSE IF (genmethod .eq. 2) THEN
           ! 20-80: use totalconc, modelv, extendlvs, sfc_mode_ratio 
           WRITE(*,*) "Generate 20-80 artificial profile"
           atm(1)%Aerosol(1)%Concentration(:)=0.
           ...
           atm(1)%Aerosol(1)%Concentration(modelv:N_LAYERS)=blvsmean
           ...
           do k=1,nblvs
              kk=N_LAYERS-k+1
              atm(1)%Aerosol(1)%Concentration(kk)=atm(1)%Aerosol(1)%Concentration(kk)+modeinc*weighting(k)
              !write(6,*) kk,atm(1)%Aerosol(1)%Concentration(kk)
           end do 

           !write(6,*) 'Finish the profile below mode'

           decaying_ratio=atm(1)%Aerosol(1)%Concentration(modelv)*0.8/abovemode

           residual=abovemode
           do k=1,modelv-1
              kk=(modelv-1)-k+1
              if (kk .eq. modelv-1) then ! 0.8 is the ratio of modelv-1 and modelv
                 atm(1)%Aerosol(1)%Concentration(kk)=atm(1)%Aerosol(1)%Concentration(modelv)*0.8
              else if (kk .lt. modelv-1) then
                 atm(1)%Aerosol(1)%Concentration(kk)=residual*decaying_ratio
              else
                 write(6,*) 'Error: kk is larger than modelv'
                 stop
              end if
              residual=residual-atm(1)%Aerosol(1)%Concentration(kk)
              !write(6,*) kk,atm(1)%Aerosol(1)%Concentration(kk)
           end do
           
           write(*,*) 'total concentration: ',totalconc,sum(atm(1)%Aerosol(1)%Concentration(:))
...
! Aerosol relative output
  if ( atm(1)%n_aerosols .ne. 0 ) then
     do i = 1, atm(1)%n_aerosols
        ...
        call check_nc( nf90_put_att(ncid,varid,"units",'K (kg/kg)^-1') )
