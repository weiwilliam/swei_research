...
EXP3='N' # Thickness (only for genmethod 1)
EXP4='Y' # Altitude
EXP5='Y' # Sfc_Mode_Ratio (SMR), available for genmethod 2 only
  ...
  if [ -s idealcrtm.nml ] ; then
     ...
     if [ $rc -eq 0 ]; then
        ...
        if [ $genmethod -eq 1 ]; then
           ...
           mv $KMODFILE $OUTDIR/Altitude/Mode_at_L${atlvl}_kmod.nc
           mv $AODKFILE $OUTDIR/Altitude/Mode_at_L${atlvl}_aodk.nc
        fi 
     elif [ $rc -ne 0 ]; then
        exit 1
     fi
